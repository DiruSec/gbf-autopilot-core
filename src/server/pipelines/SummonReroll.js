...
exports = module.exports = (
  ...
  const summons = env.summonPreferred || config.get("Summons.PreferredSummons");
  const attribute =
    env.summonAttribute || config.get("Summons.DefaultSummonAttributeTab");

  const checkLocation = () =>
    run(Location()).then(location => {
      if (isSupporterPage(location)) {
        ...
        return run(Location.Wait(pageRegexp.supporter)).then(() =>
          run(checkLocation)
        );
      }
    });
...
exports.test = (config, scenarioConfig) =>
  scenarioConfig.get("Mode") === "SummonReroll" ||
  config.get("Summons.OnlyRerollSummonsEnabled");
